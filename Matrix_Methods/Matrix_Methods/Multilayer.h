...
public:
	...
	inline double get_theta_t() { return theta_t;  } // output transmission angle
	inline double get_theta_critical() { return theta_critical;  } // output transmission angle
	inline double get_theta_brewster() { return theta_brewster;  } // output transmission angle
	inline double get_p_in() { return p_in;  }
	inline double get_p_out() { return p_out;  }

	std::complex<double> reflection(bool polarisation, double angle);
	std::complex<double> transmission(bool polarisation, double angle);
	
	double Reflectivity(bool polarisation, double angle);
	double Transmissivity(bool polarisation, double angle);

	void compute_T(bool polarisation, double n_left, double n_right, double angle);

	std::vector<std::vector<std::complex<double>>> transition_matrix();

private:
	void set_angles(double angle);
...
private:
	...
	double theta_in; // propagation angle on input side
	double theta_t; // propagtaion angle on output side, angle of transmission computed using Snell's law
	...
	double cos_theta_1; // cosine of the input propagation angle 
	double cos_theta_2; // cosine of the transmission propagation angle 
	
	double p_in; // polarisation dependent wavevector scale factor at input
	double p_out; // polarisation dependent wavevector scale factor at output
